...
r, c, m = map(int, input().split())
shark_info = [list(map(int, input().split())) for _ in range(m)]  # 상어의 위치 r, c,  상어의 속력, 방향, 크기 정보들의 배열이다.
ans = 0

def shark_move():
    global r, c, move, shark_info
    move = [[0, 0], [-1, 0], [1, 0], [0, 1], [0, -1]]
    shark_info_dict = {}
    for cur_r, cur_c, sv, dir, sz in shark_info:
        v = sv
        if dir <= 2:  # 위, 아래의 경우
            v = v % ((r - 1) * 2)
        else:
        ...
        for _ in range(v):
            ...
            cur_r += move[dir][0]
            cur_c += move[dir][1]
        pos_id = cur_r * c + cur_c
        if pos_id not in shark_info_dict:
            shark_info_dict[pos_id] = [cur_r, cur_c, sv, dir, sz]
        else:
            if shark_info_dict[pos_id][-1] < sz:
                shark_info_dict[pos_id] = [cur_r, cur_c, sv, dir, sz]
    shark_info = list(shark_info_dict.values())
            


def catch_fish(fisher_pos):
    global shark_info, ans, r
    remove_cand = sorted(filter(lambda shark: shark[1] == fisher_pos, shark_info))
    if remove_cand:
        ans += remove_cand[0][-1]
        shark_info.remove(remove_cand[0])
